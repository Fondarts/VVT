...
import { captureFrameFromVideo } from '../api/ffmpeg';
...
export const ContrastChecker: React.FC<ContrastCheckerProps> = ({ videoEl, currentTime, onContrastCheck }) => {
  ...
  const addCheck = useCallback(() => {
    ...
    // Capture current frame from the live video element (returns data URL or null)
    const thumbnailPath = videoEl ? (captureFrameFromVideo(videoEl) ?? '') : '';
  ...
  }, [textColor, bgColor, currentTime, checks, videoEl, onContrastCheck]);
